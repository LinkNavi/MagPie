      ...
      if (cit != classInfos_.end()) {
        ...
        if (fit != cit->second.fieldIndices.end()) {
          ...
          if (expr->op != AssignOp::Assign) {
            ...
            switch (expr->op) {
            case AssignOp::PlusAssign:
              finalVal = builder_.CreateAdd(current, value);
              break;
            case AssignOp::MinusAssign:
              finalVal = builder_.CreateSub(current, value);
              break;
            case AssignOp::StarAssign:
              finalVal = builder_.CreateMul(current, value);
              break;
            case AssignOp::SlashAssign:
              finalVal = builder_.CreateSDiv(current, value);
              break;
            default:
              break;
            }
...
LLVMCodeGen::generateArrayLiteralExpr(const ArrayLiteralExpr *arr) {
  if (arr->elements.empty()) {
    llvm::Type *arrType = llvm::StructType::get(
        context_, {builder_.getPtrTy(), builder_.getInt64Ty()});
    llvm::Value *result = llvm::UndefValue::get(arrType);
  ...
  llvm::Value *ptr = builder_.CreateCall(mallocFn, {totalSize}, "arr.ptr");

  for (size_t i = 0; i < count; ++i) {
    llvm::Value *elem =
        (i == 0) ? firstElem : generateExpression(arr->elements[i].get());
    if (!elem)
      continue;
    llvm::Value *elemPtr =
        builder_.CreateGEP(elemType, ptr, builder_.getInt64(i));
    builder_.CreateStore(elem, elemPtr);
  }

  llvm::Type *arrType = llvm::StructType::get(
      context_, {builder_.getPtrTy(), builder_.getInt64Ty()});
  llvm::Value *result = llvm::UndefValue::get(arrType);
  result = builder_.CreateInsertValue(result, ptr, 0);
...
inline llvm::Value *
LLVMCodeGen::generateInterpolatedStringExpr(const InterpolatedStringExpr *str) {
  std::vector<llvm::Value *> parts;
  size_t partIdx = 0;
  for (size_t i = 0; i < str->segments.size(); ++i) {
    if (!str->segments[i].empty())
      parts.push_back(createStringConstant(str->segments[i]));
    if (partIdx < str->parts.size()) {
      llvm::Value *part = generateExpression(str->parts[partIdx++].get());
      if (part && part->getType()->isPointerTy())
  ...
  llvm::AllocaInst *result =
      createEntryBlockAlloca(currentFunction_, "interp",
                             llvm::ArrayType::get(builder_.getInt8Ty(), 1024));
  llvm::Value *resultPtr = builder_.CreateBitCast(result, builder_.getPtrTy());
  builder_.CreateStore(builder_.getInt8(0), resultPtr);

  llvm::Function *strcatFn = getOrDeclareStrcat();
  for (auto *part : parts)
    builder_.CreateCall(strcatFn, {resultPtr, part});
  return resultPtr;
}
...
inline void LLVMCodeGen::generateClassConstructor(const ClassDecl *cls,
  ...
  llvm::Value *ptr = builder_.CreateCall(mallocFn, {size}, "obj");
...
inline llvm::AllocaInst* LLVMCodeGen::createEntryBlockAlloca(llvm::Function* func, const std::string& varName, llvm::Type* type) {
    llvm::IRBuilder<> tmpBuilder(&func->getEntryBlock(), func->getEntryBlock().begin());
    return tmpBuilder.CreateAlloca(type, nullptr, varName);
}

inline llvm::Value* LLVMCodeGen::castIfNeeded(llvm::Value* val, llvm::Type* targetType) {
...
inline llvm::Value* LLVMCodeGen::createStringConstant(const std::string& str) {
    return builder_.CreateGlobalStringPtr(str, ".str." + std::to_string(stringConstCounter_++));
}

inline llvm::Function* LLVMCodeGen::getOrDeclareMalloc() {
    if (auto* f = module_->getFunction("malloc")) return f;
    auto* funcType = llvm::FunctionType::get(builder_.getPtrTy(), {builder_.getInt64Ty()}, false);
    return llvm::Function::Create(funcType, llvm::Function::ExternalLinkage, "malloc", module_.get());
}

inline llvm::Function* LLVMCodeGen::getOrDeclareStrcat() {
    if (auto* f = module_->getFunction("strcat")) return f;
    auto* funcType = llvm::FunctionType::get(builder_.getPtrTy(), {builder_.getPtrTy(), builder_.getPtrTy()}, false);
    return llvm::Function::Create(funcType, llvm::Function::ExternalLinkage, "strcat", module_.get());
}
