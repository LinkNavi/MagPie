...
private:
    ...
    TypeAnnotPtr parseTypeAnnotation() {
        ...
        std::string name;
        switch (current().type) {
            case TokenType::Int8:    case TokenType::Int16:
            case TokenType::Int32:   case TokenType::Int64:
            case TokenType::UInt8:   case TokenType::UInt16:
            case TokenType::UInt32:  case TokenType::UInt64:
            case TokenType::Float32: case TokenType::Float64:
            case TokenType::Bool:    case TokenType::String_:
            case TokenType::Void:    case TokenType::Auto:
    ...
    StmtPtr parseInclude(SourceLoc loc) {
            ...
            if (!check(TokenType::RightBrace)) {
                do {
                    selectedNames.push_back(
                        expect(TokenType::Identifier, "Expected name in include list").value
                    );
                } while (match(TokenType::Comma));
            }
        ...
        return std::make_unique<IncludeDecl>(
            std::move(path), std::move(selectedNames), loc);
    }
    ...
    StmtPtr parseBlock() {
        ...
        while (!check(TokenType::RightBrace) && !isAtEnd()) {
            ...
            else break; // error already recorded
        }
    ...
    StmtPtr parseVarDecl() {
        ...
        std::string name = expect(TokenType::Identifier, "Expected variable name after 'var'").value;
        ...
        return std::make_unique<VarDecl>(
            std::move(name), std::move(typeAnnot), std::move(init), loc);
    }
    ...
    StmtPtr parseConstDecl() {
        ...
        std::string name = expect(TokenType::Identifier, "Expected constant name after 'const'").value;
    ...
    StmtPtr parseCStyleFor(SourceLoc loc) {
        ...
        return std::make_unique<ForStatement>(
            std::move(init), std::move(cond), std::move(update), std::move(body), loc);
    }
    ...
    StmtPtr parseSwitch() {
        ...
        return std::make_unique<SwitchStatement>(
            std::move(disc), std::move(cases), loc);
    }
    ...
    // --------------------------------------------------------
    StmtPtr parseFunctionDecl(std::vector<Annotation> annotations,
                              bool isPublic, bool isStatic, SourceLoc loc) {
    ...
    StmtPtr parseClassDecl(std::vector<Annotation> annotations, SourceLoc loc) {
        ...
        while (!check(TokenType::RightBrace) && !isAtEnd()) {
            ...
            // Consume visibility
            if (match(TokenType::Public))  { member.isPublic = true; }
            else if (match(TokenType::Private)) { member.isPublic = false; }

            // Consume static
            if (match(TokenType::Static)) { member.isStatic = true; }

            // Is it a method (function) or a field (var/const)?
            if (check(TokenType::Var)) {
                member.decl = parseVarDecl();
            } else if (check(TokenType::Const)) {
                member.decl = parseConstDecl();
            } else {
                // Must be a method — parse as function
                member.decl = parseFunctionDecl(
                    std::move(member.annotations),
                    member.isPublic, member.isStatic, currentLoc());
        ...
        return std::make_unique<ClassDecl>(
            std::move(name), std::move(parent),
            std::move(members), std::move(annotations), loc);
    }
    ...
    StmtPtr parseStructDecl(std::vector<Annotation> annotations) {
        ...
        return std::make_unique<StructDecl>(
            std::move(name), std::move(members), std::move(annotations), loc);
    }
    ...
    StmtPtr parseEnumDecl(std::vector<Annotation> annotations) {
        ...
        while (!check(TokenType::RightBrace) && !isAtEnd()) {
            ...
            entry.name = expect(TokenType::Identifier, "Expected enum entry name").value;
        ...
        return std::make_unique<EnumDecl>(
            std::move(name), std::move(entries), std::move(annotations), loc);
    }
    ...
    //  10  unary          !  - (prefix)
    //  11  postfix        ()  []  .  .?
    //  12  primary        literals, identifiers, grouping, lambda, new, this, super
    // ============================================================
    ...
    static int binaryPrecedence(TokenType type) {
        switch (type) {
            ...
            case TokenType::SlashAssign:   return PREC_ASSIGN;

            case TokenType::QuestionQuestion: return PREC_NULL_COALESCE;
            case TokenType::Or:               return PREC_OR;
            case TokenType::And:              return PREC_AND;
            case TokenType::Equal:
            case TokenType::NotEqual:         return PREC_EQUALITY;
            case TokenType::Less:
            case TokenType::LessEqual:
            case TokenType::Greater:
            case TokenType::GreaterEqual:     return PREC_COMPARISON;
            case TokenType::DotDot:
            case TokenType::DotDotDot:        return PREC_RANGE;
            case TokenType::Plus:
            case TokenType::Minus:            return PREC_ADDITION;
            case TokenType::Star:
            case TokenType::Slash:
            case TokenType::Percent:          return PREC_MULTIPLICATION;
            default:                          return 0;
        }
    }
    ...
    ExprPtr parseExpression() {
        return parseExprPrec(PREC_ASSIGN);
    }
    ...
    ExprPtr parseExprPrec(int minPrec) {
        ...
        while (true) {
            ...
            // For right-associative ops, the right side parses at the same precedence.
            // For left-associative ops, we go one higher to enforce left grouping.
            int nextPrec = isRightAssoc(opType) ? prec : prec + 1;

            ExprPtr right = parseExprPrec(nextPrec);
            if (!right) return left; // error already recorded
    ...
    // --------------------------------------------------------
    ExprPtr buildBinaryNode(TokenType opType, ExprPtr left, ExprPtr right, SourceLoc loc) {
        switch (opType) {
            // Assignment operators → AssignExpr
            case TokenType::Assign:
                return std::make_unique<AssignExpr>(AssignOp::Assign, std::move(left), std::move(right), loc);
            case TokenType::PlusAssign:
                return std::make_unique<AssignExpr>(AssignOp::PlusAssign, std::move(left), std::move(right), loc);
            case TokenType::MinusAssign:
                return std::make_unique<AssignExpr>(AssignOp::MinusAssign, std::move(left), std::move(right), loc);
            case TokenType::StarAssign:
                return std::make_unique<AssignExpr>(AssignOp::StarAssign, std::move(left), std::move(right), loc);
            case TokenType::SlashAssign:
                return std::make_unique<AssignExpr>(AssignOp::SlashAssign, std::move(left), std::move(right), loc);

            // Null coalescing → NullCoalesceExpr
            case TokenType::QuestionQuestion:
                return std::make_unique<NullCoalesceExpr>(std::move(left), std::move(right), loc);

            // Range → RangeExpr
            case TokenType::DotDot:
                return std::make_unique<RangeExpr>(std::move(left), std::move(right), true, loc);
            case TokenType::DotDotDot:
                return std::make_unique<RangeExpr>(std::move(left), std::move(right), false, loc);

            // Everything else → BinaryExpr
            default:
                return std::make_unique<BinaryExpr>(tokenToBinaryOp(opType), std::move(left), std::move(right), loc);
        }
    }
    ...
    ExprPtr parseUnary() {
        ...
        if (check(TokenType::Minus)) {
            ...
            return std::make_unique<UnaryExpr>(UnaryOp::Negate, std::move(operand), loc);
        }
    ...
    ExprPtr parsePostfix() {
        ...
        while (true) {
            if (check(TokenType::LeftParen)) {
                ...
                expr = std::make_unique<MemberExpr>(std::move(expr), std::move(member), loc);
            }
                ...
                advance(); // consume .?
                std::string member = expect(TokenType::Identifier, "Expected member name after '.?'").value;
                expr = std::make_unique<OptionalMemberExpr>(std::move(expr), std::move(member), loc);
            }
            else {
                break;
            }
    ...
    ExprList parseArgumentList() {
        ExprList args;
        if (check(TokenType::RightParen)) return args; // empty
    ...
    ExprPtr parsePrimary() {
        ...
        switch (current().type) {
            ...
            case TokenType::Integer: {
                ...
                return std::make_unique<LiteralExpr>(LiteralKind::Integer, std::move(val), loc);
            }
            case TokenType::HexInteger: {
                std::string val = advance().value;
                return std::make_unique<LiteralExpr>(LiteralKind::HexInteger, std::move(val), loc);
            }
            case TokenType::BinaryInteger: {
                std::string val = advance().value;
                return std::make_unique<LiteralExpr>(LiteralKind::BinaryInteger, std::move(val), loc);
            }
            case TokenType::Float: {
                std::string val = advance().value;
                return std::make_unique<LiteralExpr>(LiteralKind::Float, std::move(val), loc);
            }
            case TokenType::Scientific: {
                std::string val = advance().value;
                return std::make_unique<LiteralExpr>(LiteralKind::Scientific, std::move(val), loc);
            }

            // --- String literals ---
            case TokenType::String: {
                std::string val = advance().value;
                return std::make_unique<LiteralExpr>(LiteralKind::String, std::move(val), loc);
            }
            case TokenType::RawString: {
                std::string val = advance().value;
                return std::make_unique<LiteralExpr>(LiteralKind::RawString, std::move(val), loc);
            }
            ...
            case TokenType::Char: {
                std::string val = advance().value;
                return std::make_unique<LiteralExpr>(LiteralKind::Char, std::move(val), loc);
            }
    ...
    ExprPtr parseNew() {
        ...
        std::string className = expect(TokenType::Identifier, "Expected class name after 'new'").value;
        ...
        return std::make_unique<NewExpr>(std::move(className), std::move(args), loc);
    }
    ...
    ExprPtr parseLambda(SourceLoc loc) {
        ...
        return std::make_unique<LambdaExpr>(
            std::move(params), std::move(bodyExpr), std::move(bodyBlock), loc);
    }
    ...
    ExprPtr parseInterpolatedString(const std::string& raw, SourceLoc loc) {
            ...
            if (raw[i] == '$' && i + 1 < raw.size() && raw[i + 1] == '{') {
                ...
                if (subParser.hasErrors() || !subExpr) {
                    ...
                    parts.push_back(std::make_unique<LiteralExpr>(
                        LiteralKind::String, exprSource, loc));
                } else {
        ...
        return std::make_unique<InterpolatedStringExpr>(
            std::move(segments), std::move(parts), loc);
    }
